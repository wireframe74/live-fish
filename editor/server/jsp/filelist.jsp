...
<%
  ...
  for(int i=0;i<currentFilter.length;i++) {
    isAllowed = false;
    for(int j=0;j<allFilters.length;j++) {
      if(currentFilter[i].equals(allFilters[j])) {
        isAllowed = true;
      }
    }
    if(!isAllowed) {
	...
	if(!isAllowed) {
		out.print("Filter error ! <br><br>Please configure editor/config/security.xml.");
		return;
	}
  //------------------------------------------------------------------------
  // Security check
...
<html>
<head>
<script language="javascript" src="../../config/localization/en.js"></script>
<script>
var selectedRow = -1;
var selectedType = "";
var mode = "<%=mode %>";
