...
try {
	...
	if (userSession == null) {
		...
		<form name="qtyform" method="post" action="detail.jsp?rpid=<%=pageModel.getRecParentVendorArticleId()%>&amp;rid=<%=pageModel.getRecVendorArticleId()%>">
			
			<input type="hidden" name="action" value="" />			
				
			<% if (!pageModel.isHasOptions() && pageModel.getOptionSetId() == 0 && !pageModel.isOption()) { %>
			...
			<% } else if (pageModel.isHasOptions() || pageModel.isOption()) { %>
			...
			<% if ((pageModel.isHasOptions() || pageModel.isOption()) || pageModel.getOrderType() == VendorArticle.ORDER_TYPE_ENQUIRIES_ONLY) { %>
			...
			<% if (!pageModel.isHasOptions() || pageModel.isOption()) { %> 
						...
						<% if (userSession.hasActiveGiftList() && userSession.isGiftListOwner()) { %>
							...
							<% } else {	%> 
								...
								<div class="addtobasketqty">
									...
									<a class="detailquantityplus" href="javascript:qty_plus('qtyPlusMinus<%=seq%>', <%=pageModel.getQuantity()%>)"><%=website.getText("detailquantityplus","+")%> </a>
								</div>


								<a class="buttonaddtobasket"  href="javascript: document.qtyform.action.value='glAdd';document.qtyform.submit();">
									<%=website.getText("detailAddtogiftlist","<span>Add to Giftlist</span>")%>
								</a>
				...
				<ul class="detailextras">
					<% if (website.isOfferGiftWrappingService() && true ) { %>
						<li>
						   <a id="extrasgiftwrap" class="extras" href="#div_giftwrapoptions" onclick='javascript:document.getElementById("div_giftwrapoptions").style.display="";' >
							<span class="extrapreplus"> <%=website.getText("detailgiftwrapping+","+")%> </span>
					...
					<% if (website.isShowPersonalisationForms() && pageModel.getPersonalisationFormName() != null) { %>
						<li>
						   <a class="extras" id="extraspersonalisation" href="#personalisationoptions" >
							<span class="extrapreplus"> <%=website.getText("detailpersonalisation+","+")%> </span>  
							<span class="extrastext"> <%=website.getText("personalisationOptions","Personalisation")%> </span>
						   </a>
						</li>
					<% }else if(website.isOfferWritingService()){ %>
						<li>
							 <a class="extras" id="extrahandwritecard" href="#div_giftwrapoptions" onclick='javascript: document.getElementById("div_giftwrapoptions").style.display = "";'>
								<span class="extrapreplus"> <%=website.getText("detailhanwritecard+","+")%> </span>
					...
					<% if (website.isShowTellAFriend()) { %>
						<li>
							<a href="#tellafriend" class="extras" id="extrastellafriend" onclick='javascript: document.getElementById("div_tellafriend").style.display = "";document.tellafriendform.tellyourname.focus();'>
								<span class="extrapreplus"> <%=website.getText("detailtellafriend+","+")%> </span>
					...
					if( false && website.isOfferGiftWrappingService() ){ %>
						<li>
							<a class="extras" onClick="toggleGiftwrap()" >
								<span class="extrapreplus"><%=website.getText("detailpersonalisation+","+")%></span>
								<span class="extrastext"><%=website.getText("handwritecard1","Giftwrapping")%></span>
							</a>
						</li>
					<%}
					if( false &&  website.isOfferWritingService() && ( !website.isShowPersonalisationForms() || pageModel.getPersonalisationFormName() == null ) ){ %>
						<li>
							<a class="extras" onClick="toggleHandwrite()" >
								<span class="extrapreplus"><%=website.getText("detailpersonalisation+","+")%></span>
								<span class="extrastext"><%=website.getText("handwritecard2","Handwritten card")%></span>
							</a>
						</li>
					<%}%>
					...
					<ul class="detailextras">
						<li>
							<a href="#tellafriend" onclick='javascript: document.getElementById("div_tellafriend").style.display = "";document.tellafriendform.tellyourname.focus();'>
								<%=website.getText("detailtellafriend+","+")%>
