...
try {
	...
	if (userSession == null) {
		...
		String dimenionStyleHTML = website.isProductPageFixedWidthInCSS()?"style=\"overflow:hidden;display:block;"+(maxImageHeight>0?"height:"+maxImageHeight+"px;min-height:"+maxImageHeight+"px;":"")+"\"":"style=\"height:"+website.getProductPageImageSize()+"px;width:"+website.getProductPageImageSize()+"px;\"";%>
			...
			<div id="ajax_image_wrap" class="componentBorder">
				...
				<% if (currentImage != null) { %>
					...
					<% 	}
				 	...
				 	%>		
							      <a id="mainZoomer" href="<%=largeImageNamePrimary%>" class="productMainLink MagicZoomPlus" rel="<%=website.getZoomImageSettings()%>" >
								
					<%		} else { %>
							      <a id="mainZoomer" href="<%=largeImageNamePrimary%>" class="productMainLink MagicZoomPlus" rel="<%=website.getZoomImageSettings()%>" alt="<%=currentImage.getImageShortDescription()%>" > 
