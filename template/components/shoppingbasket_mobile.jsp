			...
			<div class="pagetopnav">
				...
				<ul class="navPage blocklistright">
					<% if (userSession.getShoppingBasket().getTotalQuantity() > 0) { %>
						<li class="buttoncheckout firstbutton" >
...
<div class="pagetitle"><h1><%=website.getText("basketheaderone"," My Basket")%></h1></div>
<div class="pagesubtitle" id="basketitemsheader"><h2><%=userSession.getShoppingBasket().getTotalQuantityString()%> <%=website.getText("basketsubheaderone"," Items")%></h2></div>
...
<%
		...
		for (int i=0;i<consoleTableModel.getRowCount();i++) {
			try {
				if (!discountManager.isDiscountRow(i)) {
					...
					<% } %>
					<ul class="mob_basketline listtableline">
								...
								<li class="basketimage">
									<a class="smallertext"  href='<%=ArticleMenu.getPageName(website, shortDescription, true)%>?pid=<%=parentVendorArticleId%>&amp;id=<%=vendorArticleId%>&amp;tlid=<%=transLineId%>'>
										<img src="<%=imageName %>" alt="<%=shortDescription %>" title="<%=shortDescription %>" />
									</a>
								</li>
								<% } %>
					 <li class="basketitemright">
						<ul>
					...
					<li class="basketitemfunctions">
						<ul>
							<%if (website.isOfferGiftWrappingService() && (wrappingTypeDomainId == 0 || wrappingTypeDomainId == TransactionLine.WRAPPING_TYPE_NO_WRAPPING)) {%>											
								<li>
							...
							<li class="basketupdate">
								<% if (!website.isAllwaysShowInStock() && userSession.getArticleDetail().getOrderType(vendorArticleId)==VendorArticle.ORDER_TYPE_SELL_IN_STOCK_ONLY) { %>
									<a href=<%="'javascript: if (ValidateQty(document.qtyform"+i+".qty.value, document.qtyform"+i+".sqty.value, document.qtyform"+i+".aibqty.value, document.qtyform"+i+".origqty.value)) document.qtyform"+i+".submit();'"%> class="smalltext"><%=website.getText("basketupdateline","Update")%></a>
								<% } else { %>
									<a href=<%="'javascript: document.qtyform"+i+".submit();'"%> class="smalltext"> <%=website.getText("basketupdateline","Update")%> </a>
								<% } %>
							</li>
		...
		<% } if (userSession.getWebsiteAccessDomainId() > Customer.WEB_ACCESS_TYPE_NO_PRICES) { %>
			<li>
				<ul class="majorbasketsubtotal labelpairleft">				
					<li class="subtotaltitle"> Sub Total </li>
	...
	<% if (website.isShowDelivery()) {%>
		<div id="basketdelivery">
			<h2> <%=website.getText("basketheadertwomob","Delivery")%> </h2>
	...
	<% if (userSession.getWebsiteAccessDomainId() > Customer.WEB_ACCESS_TYPE_NO_PRICES) { %>
		<div id="ordertotal ">
			<ul class="majorbaskettotal labelpairleft">
				<li class="totaltitle"> <%=website.getText("basketsubdeliverymob","Order Total")%> </li>
...
<div id="voucherandpromo">
	<form name="discountform" action="basket.jsp" method="post">
		<h2> <%=website.getText("basketheaderthreemob","Promotional Codes or Vouchers")%> </h2>
			<ul class="vandp">
				<li class="promocodes">	
					<ul>
