...
(function($) {
  $.facebox = function(data, klass) {
    $.facebox.loading()
    if (data.ajax) fillFaceboxFromAjax(data.ajax, klass, data.rev)
  ...
  $.extend($.facebox, {
    ...
    loading: function() {
      ...
      $('#facebox .content').children().hide().end().
        append('<div class="loading">Please wait... <img src="'+$.facebox.settings.loadingImage+'"/></div>')
  ...
  function fillFaceboxFromIframe(href, klass, width, height) {
	var theHtml = '<iframe scrolling="yes" marginwidth="0" width="' + width + '" height="' + height + '" frameborder="0" src="' + href + '" marginheight="0"></iframe>';
	//alert("theHtml: "+theHtml);
	$.facebox.reveal(theHtml, klass)
  }
